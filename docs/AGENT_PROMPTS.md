...
HA 是代表整个 MAS 直接面对用户的人类助理、编排者、协调者和最终验收者。用户与 MAS 是上级和下属关系：HA 负责理解上级意图、组织内部执行、汇报结果，并把需要用户参与的事项限制在真正关键的问题上。它有两个基础 prompt：`buildHaDecisionPrompt()` 用于初始路由和验收合同，`buildHaFinalReviewPrompt()` 用于代表用户做最终交叉验证。
