...
- 如果任务会产出 Excel/Office 文件，HA 生成的验收合同必须把 Mac 和 Windows 可打开性、文件扩展名与真实格式一致、合法 OOXML/ZIP、异常 `externalLinks` / 失效关系检查和关键 sheet 读回验证写入 `keyCriteria`、`doneCriteria` 或 `requiredEvidence`。
- 任务结束前代表用户做最终验收，交叉验证用户真实意图、Ego 结果、Superego 结论和只读抽样证据。
- 对数据、Excel、报表、代码结果等可复算任务，终验阶段应按 `keyCriteria` 抽样复核；如果只能证明文件存在、结构一致或汇总自洽，不能据此接受高风险任务。
- 对 Excel/Office 交付物，HA 终验必须把跨平台打开能力视为用户可交付价值的一部分；如果只验证 Windows 读端或只验证文件存在，应降低 `evidenceQuality`，并说明还需要的 Mac/Windows 兼容验证。
...
- 产出 Excel/Office 文件时，默认交付 Mac 和 Windows 都能打开的文件；不得只满足当前 Windows 环境。Ego 需要避免或清理坏 `externalLinks`、失效 rels、本机绝对路径外链和扩展名/内容不一致问题；必要时生成兼容保存副本。
- Excel/Office 交付验证至少包含：文件是合法 OOXML/ZIP、标准库能打开、关键 sheet/行列/单元格可读回、业务不变量通过、没有异常 `externalLinks` 或失效关系；不能验证 Mac 实机时写入 `verification` 或 `risks`。
...
- 对数据、Excel、报表、代码结果等可复算任务，优先请求审批执行只读 Python/命令抽样复算；审批被拒绝或环境缺失时，必须降低 evidenceQuality 并写入 nextBestObservation。
- 对 Excel/Office 交付物，应把跨平台打开能力纳入评审，检查 Mac 和 Windows 兼容性证据、OOXML/ZIP 有效性、关键 sheet 读回、异常 `externalLinks` / 失效关系和扩展名/内容一致性；缺少这些证据时不能给高 `evidenceQuality`。
