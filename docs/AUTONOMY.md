...
默认验收策略是“当前状态门禁 + 历史事实留痕”。当前仍存在违反用户/验收合同声明的允许输出边界、只读输入路径写入或失败验证伪装为成功时，即使模型评审返回 `accept`，MAS 审计门禁也会把结论改为 `revise`。未显式要求 `output/` 时，greenfield 项目源码、文档和配置可以写在 workspace 根目录内；历史已清理的越界写入和 `changed_files` 漏报会保留在审计包中，要求 Superego 记录和评估修复是否充分，但不单独作为永久阻塞。
...
文件系统 snapshot/diff 必须谨慎使用：默认不做全量工作区扫描或全量 hash，而是对用户声明的只读输入边界、允许输出边界和已知写入路径做轻量元数据对账。只有出现命令副作用、审计矛盾、返工失败或高风险数据任务时，才触发 hash 或内容级深查。
